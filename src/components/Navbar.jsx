...
import { Home, User, FolderGit2, Trophy, Mail } from 'lucide-react';

const Navbar = () => {
  const links = [
    { to: '/', label: 'Home', icon: <Home size={22} /> },
    { to: '/about', label: 'About', icon: <User size={22} /> },
    { to: '/projects', label: 'Projects', icon: <FolderGit2 size={22} /> },
    { to: '/achievements', label: 'Achievements', icon: <Trophy size={22} /> },
    { to: '/contact', label: 'Contact', icon: <Mail size={22} /> }
  ];
        ...
        {links.map(link => (
          <li key={link.to} className={styles.navItem}>
            ...
            >
              ...
              <div className={styles.tooltip}>{link.label}</div>
            </NavLink>
          </li>
        ))}
      </ul>
